...
async fn map_operations() {
    ...
    assert_eq!(*rec.bins.get(bin_name).unwrap(), windpike::list!(5, 12));
    ...
    let val = windpike::map!("a" => 1, "b" => 2, "c" => 3, "d" => 4, "e" => 5);
    let bin_name = "bin";
    let bin = Bin::new(bin_name, val);
    let bins = vec![bin];
    ...
    let mval = windpike::map!("x" => 7, "y" => 8, "z" => 9);
